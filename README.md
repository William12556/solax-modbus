...
Installs to `/opt/solax-monitor/`. A symlink is created at `/usr/local/bin/solax-monitor` so the command is available without a full path. The symlink is verified on each install and corrected if stale. Run the installer as root (`sudo`).
...
| `--no-serve`         | Disable the Web UI (enabled by default)          |
| `--http-port PORT`   | Web UI port (default: 8181)                      |
| `--allow CIDR`       | Restrict Web UI access to a network (repeatable) |

Example:

```bash
sudo ./install.sh --ip 192.168.1.100 --http-port 8181
```
...
HTTP interface providing live telemetry in a browser, alongside the console display. Enabled by default.

**Disable:**

```bash
solax-monitor <INVERTER-IP> --no-serve
```
...
| 1.10 | 2026-07-07 | Added `sudo` to all install/update commands (installer requires root). Web UI now enabled by default: `--serve` replaced by `--no-serve`; default Web UI port 8080 -> 8181. See change-a7c3e9d2. |
