...
```
  ...
  - P00.17 Templates
    - All templates are external documents in ai/governance/software-engineering/templates/ directory
    - Template files:
      - ai/governance/software-engineering/templates/T02-design.md
      - ai/governance/software-engineering/templates/T07-change.md
      - ai/governance/software-engineering/templates/T06-issue.md
      - ai/governance/software-engineering/templates/T03-prompt.md
      - ai/governance/software-engineering/templates/T04-test.md
  ...
  - P00.18 Skills Management (Claude Code profiles only)
    ...
    - Mandatory skill:
      - ai/governance/software-engineering/skills/validation/run-tests.md: PostToolUse pytest execution (P15.15); required for claude_code and claude_omlx target profiles
  ...
  - P00.19 Context Optimization
    ...
    - Engine auto-load: worker recipe reads project_root/ai/context.md at task start if present
    - Content specification:
      - Project overview and technology stack
      - Common bash commands (build, test, lint)
      - Code style guidelines
      - Repository conventions (branch naming, commit patterns)
      - Governance framework location: ai/governance/software-engineering/governance.md
...
## P02 Audit
  ...
  - P02.9 Audit Modes
    ...
    - P02.9.2 Tactical audit
      ...
      - Procedure: ai/governance/software-engineering/doc/guide-audit-loop.md
...
## P04 Change
  ...
  - P04.12 Trivial Change Exemption
    ...
    - If any criterion fails, standard T06 → T07 → T03 → engine workflow applies.
...
```
    └── <project name>/
        ...
        │   ├── engine/               # Framework-owned: orchestrator, recipes, engine-mcp
        │   ├── governance/
        │   │   └── software-engineering/  # Framework-owned: governance model (this document, templates)
        ...
        │   └── validation/           # Skill docs provisioned from ai/governance/software-engineering/skills/, git-tracked
...
```
  - P10.8 Implementation Profile Setup (Human executes)
    ...
    - **engine-mcp setup (Claude Desktop profile, optional)**:
      - Server: `ai/engine/mcp/server.py` in the AI-G&O clone (versioned with the engine; replaces the separate ael-mcp repository)
      - Interpreter: the engine's Python environment (dependencies from `ai/engine/requirements.txt`); the server launches the orchestrator with the same interpreter
      - Add to `~/Library/Application Support/Claude/claude_desktop_config.json`:
```json
{
  "mcpServers": {
    "engine-mcp": {
      "command": "/path/to/engine-venv/bin/python",
      "args": ["/path/to/AI-Governance-and-Orchestration/ai/engine/mcp/server.py"]
    }
  }
}
```
      - Replace both paths with the engine environment's interpreter and the local clone path
      - Restart Claude Desktop to load the new MCP server
      - Reference: P00.11
    - **Engine setup (both profiles)**:
...
```
  - P15.15 Progressive Validation Strategy
    ...
    - **Validation Hooks (Tactical Domain, claude_code/claude_omlx profiles — mandatory):**
      ...
      - Code validation: Executes pytest for modified component; canonical template ai/governance/software-engineering/skills/validation/run-tests.md (P00.18)
      - Hook configuration: Provisioned from ai/governance/software-engineering/skills/validation/ into .claude/hooks/ and .claude/settings.json per P10.8
