...
| ------------ | ----------------------------------- | ----------------------------- | ----------------------------------- |
| Execution    | Automated loop                | Manual                        | Manual                              |
...
**overwatch** — a read-only monitor that renders a downstream project's
governance state to a self-contained browser page. Run from the project root:
...
violations, `tactical_brief` validity, naming convention) and a document
registry grouped by UUID. Writes `overwatch.html` (project root) and
`ai/dashboard-alerts.md` each scan cycle. govwatch (TUI) is retired
(change-5bcd46ad).
...
- T03 prompt authoring and engine command delivery (P13)
- Context budget check before every engine-targeted T03 prompt
- Human handoff: ready-to-execute command after approval

**Quality**

- Code review after engine `SHIP` (or Claude Code completion)
- Test documentation and pytest generation (P15)
- Configuration audit against design baseline (P02)

**Issue Management**

- T06 issue creation from failed tests or engine `BLOCKED` (P03)
...
P00  Close documents         →  move to closed/ → git commit → engine reset
```
...
| Tactical | "conduct a tactical audit" | Engine audit loop | Prepare `audit-uml.md` + `audit-index.md` + audit T03 prompt; human approval; launch per `ai/governance/software-engineering/doc/guide-audit-loop.md` |
...
| P14 | Quality | Code validation, automated audits via engine hooks |
| P02 | Audit | Strategic or tactical audit (§4.1); findings → issues (P03) |
| P13 | Prompt | T03 authoring, `tactical_brief`, engine command delivery |
...
- Before authoring any engine-targeted T03 prompt (prompt_info.target_profile:
  engine), call `omlx_model_status` (mcp_omlx) for the configured model; a null
  or missing `settings.max_context_window` is unresolved — warn the operator,
  same as the resolver's own unknown-window behavior.
- Read `context-budget.md` (written automatically by the orchestrator at engine
...
- Qualifies only when both trivial AND surgical, and all five criteria met:
  ...
  human-approved.
- Exempt from T06 → T07 → T03 → engine. Git commit is the sole audit record.
...
- The T06 → T07 corrective loop is triggered only by engine `BLOCKED` or test failure.
...
All templates reside in `ai/governance/software-engineering/templates/`. Read the template before creating
any document.

| Template | Document Class | Location |
|---|---|---|
| T02 | Design | `ai/governance/software-engineering/templates/T02-design.md` |
| T07 | Change | `ai/governance/software-engineering/templates/T07-change.md` |
| T06 | Issue | `ai/governance/software-engineering/templates/T06-issue.md` |
| T03 | Prompt | `ai/governance/software-engineering/templates/T03-prompt.md` |
| T04 | Test | `ai/governance/software-engineering/templates/T04-test.md` |
| T05 | Result | `ai/governance/software-engineering/templates/T05-result.md` |
| T01 | Requirements | `ai/governance/software-engineering/templates/T01-requirements.md` |
| T08 | Audit | `ai/governance/software-engineering/templates/T08-audit.md` |
